...
/* Η MAC του Sender (Αυτήν που "ακούει" ο Receiver) */
static const uint8_t SENDER_MAC[] = {0x1a, 0x00, 0x00, 0x00, 0x00, 0x00};

/* Η MAC του Receiver (Unicast). Αν δεν την ξέρεις, άσε FF:FF:FF:FF:FF:FF (Broadcast) */
static const uint8_t TARGET_RECEIVER_MAC[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

static const char *TAG = "csi_send";

/* ✅ MAGIC HEADER PAYLOAD: Αντί για χύμα int, στέλνουμε υπογραφή */
...
#endif
{
    // Silent drop warning for ISR stability
}
...
void app_main(void) {
    ...
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ...
        if (reason == ESP_RST_PANIC || reason == ESP_RST_WDT) {
            ...
            while(1) { vTaskDelay(pdMS_TO_TICKS(1000)); } // Safe Halt
        }
    ...
    ESP_LOGI(TAG, "Channel: %d | Freq: %d Hz", WIFI_CHANNEL, CONFIG_SEND_FREQUENCY);

    uint32_t fail_count = 0;
    uint32_t count = 0;
    int64_t next_send_us = esp_timer_get_time();
    radar_payload_t payload;
    payload.magic = RADAR_MAGIC_SIGNATURE;
    ...
    while (true) {
        ...
        ret = esp_now_send(peer.peer_addr, (const uint8_t *)&payload, sizeof(payload));
        ...
        if (sleep_us > 0) {
            ...
            next_send_us = esp_timer_get_time(); // Reset timer if lagging
        }
    }
}
...
from datetime import datetime # <--- Προστέθηκε για το timestamp!
...
class csi_data_graphical_window(QWidget):
    def __init__(self):
        ...
        self.plotWidget_multi_data.setTitle('Subcarrier Phase Data')  
        self.plotWidget_multi_data.setLabel('left', 'Phase (rad)')  
        self.plotWidget_multi_data.setLabel('bottom', 'Time (Cumulative Packet Count)')  
...
if __name__ == '__main__':
    ...
    # --- ΠΡΟΣΘΗΚΗ ΤΗΣ ΩΡΑΣ (Timestamp) ---
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    default_csv = f"./csi_data_{current_time}.csv"
    default_log = f"./csi_data_log_{current_time}.txt"

    parser = argparse.ArgumentParser(
        description='Read CSI data from serial port and display it graphically')
    parser.add_argument('-p', '--port', dest='port', action='store', required=True,
                        help='Serial port number of csv_recv device')
    # Αντικατάσταση των default ονομάτων με αυτά που έχουν το timestamp
    parser.add_argument('-s', '--store', dest='store_file', action='store', default=default_csv,
                        help='Save the data printed by the serial port to a file')
    parser.add_argument('-l', '--log', dest='log_file', action='store', default=default_log,
    ...
    print(f"📡 Ξεκίνησε η καταγραφή της Espressif...")
